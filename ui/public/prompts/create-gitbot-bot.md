...
Work through the steps in order. Do not skip the interview, and do not write anything to disk
until I have seen the definition and approved it.

---

## 1. Interview me

I probably have a rough idea, not a specification. Draw it out.

Ask about two or three things at a time, not everything at once. Where you can reasonably infer
an answer, propose it and let me correct you — "I'm assuming this should never push to main, tell
me if that's wrong" beats an open question.

Find out:

- **The job.** What should this bot do, every single time, without being told again? If the answer
  is a one-off task, this should not be a bot — say so.
- **The trigger.** What does a typical conversation with it look like? I might open a thread and
  say nothing more than "hi", so the job has to be clear enough to start from that alone.
- **The input.** What does it look at — a diff, a folder of files, an issue, a failing test run?
  Where does it find that without me telling it?
- **The output.** What has changed when it is finished? A written report in the chat, edited
  files, a commit, a PR, a comment posted somewhere?
- **The limits.** What must it never do? Touch main, force-push, install packages, edit files
  outside a directory, contact anything external.
- **Where it runs.** Is this for one specific repo of mine, or for any repo of a given shape (a
  Node project, a Rust crate, anything with a `CLAUDE.md`)? A bot that only works in one checkout
  is fine, but say so in the description.
- **What the machine needs.** Any tool, CLI, login or install step that has to exist before the
  job is possible — `gh` authenticated, `ffmpeg` on PATH, dependencies installed.
...
## 3. Write the setup steps — only if they are really needed
...
configured. "Works in any Node repo" needs no setup.
...
**Agent** — one of `claude-code`, `opencode`, `codex`. Default to `claude-code` unless I say
otherwise. The differences that matter:
...
`opencode`, in `provider/model` form, e.g. `anthropic/claude-haiku-4-5`.
...
## 5. Show me the definition before writing anything
...
## 6. Write it to gitbot's data file
